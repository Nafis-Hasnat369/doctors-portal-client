...
const AllUsers = () => {
    const { data: users = [], refetch, isLoading } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch(`https://doctors-portal-server-six-eosin.vercel.app/users`);
    ...
    const handleMakeAdmin = id => {
        fetch(`https://doctors-portal-server-six-eosin.vercel.app/users/admin/${id}`, {
            method: 'PUT',
    ...
    const handleDelete = id => {
        fetch(`https://doctors-portal-server-six-eosin.vercel.app/delete/${id}`, {
            method: 'DELETE',
            headers: { authorization: `bearer ${localStorage.getItem('accessToken')}` }
        })
            .then(res => res.json())
